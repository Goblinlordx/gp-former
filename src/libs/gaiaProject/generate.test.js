...
it("returns same result given same seed", async () => {
  const result1 = generate({ seed: 0 });
  const result2 = generate({ seed: 0 });
  expect(result1).toEqual(result2);
});

it("result has 1 federation token different per seed", async () => {
  const result1 = await generate({ seed: 0 });
  const result2 = await generate({ seed: 100 });
